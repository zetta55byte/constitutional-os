...
class GovernanceClient:
    ...
    def __init__(
        self, url: str = "https://constitutional-os-production.up.railway.app"
    ):
        self.url = url
    ...
    def _evaluate_membranes(self, action: dict) -> dict:
        ...
        if scope == "constitutional" and autonomy != "human-directed":
            return self._block(
                "M1_safety", "Constitutional-scope change requires human direction."
            )

        # M2 Reversibility
        if not reversible and autonomy == "autonomous":
            return self._defer(
                "M2_reversibility", "Irreversible autonomous action deferred."
            )
        ...
        if any(t in delta_type for t in lock_in):
            return self._block(
                "M3_pluralism", "Action would eliminate future option space."
            )
        ...
        if autonomy == "autonomous" and (
            ...
            return self._defer(
                "M4_human_primacy",
                "Significant autonomous action deferred for human review.",
            )
    ...
    def print_chain(self, n: int = 5):
        ...
        for e in entries:
            icon = (
                "✓"
                if e["verdict"] == "PASS"
                else ("✗" if e["verdict"] == "BLOCK" else "⏸")
            )
            print(
                f"  [{e['seq']:>3}] {icon} {e['verdict']:<6} │ {e['action'][:40]:<40} │ {e['ts'][:19]}"
            )
        print("─────────────────────────────────────────────────────────\n")
...
class GovernedResearchAgent:
    ...
    def run_demo(self):
        ...
        result = self.cite(
            "Lyapunov stability in governance systems", "Constitutional OS, Zenodo 2026"
        )
        print(f"   → {result}")
        print()
