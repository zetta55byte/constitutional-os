...
def diff_profiles(old: Profile, new: Profile) -> ProfileDiff:
    ...
    parts = []
    if added:
        parts.append(f"+{len(added)} metrics")
    if removed:
        parts.append(f"-{len(removed)} metrics")
    if changed:
        parts.append(f"~{len(changed)} metrics changed")
    if d.added_evals:
        parts.append(f"+{len(d.added_evals)} evals")
    if d.removed_evals:
        parts.append(f"-{len(d.removed_evals)} evals")
    if d.config_changed:
