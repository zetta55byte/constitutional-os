...
def boot(
    ...
    from constitutional_os.actions.deltas import ContinuityLog, DeltaEngine, LogEntry
    from constitutional_os.evals.runner import EvalHistory, EvalRunner
    from constitutional_os.forecast.engine import ForecastEngine, ForecastState
    ...
    def on_eval_completed(state, event: EvalCompleted):
        if not event.passed:
            log(
                f"Eval FAILED: {event.bundle_id}/{event.profile_id} score={event.score:.2f}"
            )
        return state, []
    ...
    def on_action_recommended(state, event: ActionRecommended):
        ...
        log(
            f"ActionRecommended [{event.urgency}]: {event.delta_type} — {event.rationale[:60]}"
        )
        ...
        proposed = ProposedDelta(
            ...
            autonomy="autonomous",
            severity=(
                "significant" if event.urgency in ("high", "critical") else "normal"
            ),
            reversible=True,
            scope="local",
        ...
        if mem_result.verdict == MembraneVerdict.BLOCK:
            entry = LogEntry(
                ...
                rationale=f"Blocked: {mem_result.blockers}",
            )
        ...
        if mem_result.verdict == MembraneVerdict.DEFER:
            entry = LogEntry(
                ...
                status="deferred",
                author="reliability_os",
                rationale=f"Deferred: {mem_result.deferrals}",
            )
            state.actions_log.append(entry)
            return state, [
                HumanVetoWindowOpened(
    ...
    def on_human_veto_opened(state, event: HumanVetoWindowOpened):
        log(
            f"Human veto window opened: proposal {event.proposal_id} ({event.window_secs}s)"
        )
        return state, []
