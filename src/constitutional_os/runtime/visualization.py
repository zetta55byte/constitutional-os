...
def plot_lyapunov_trajectory(
    records: list[
        dict
    ],  # list of {cycle, v, v_inv, v_mem, v_drift, v_rec, verdict, fixed}
    ...
    [r.get("v_rec", 0) for r in records]

    fig, (ax1, ax2) = plt.subplots(
        2, 1, figsize=cfg.figsize, gridspec_kw={"height_ratios": [2, 1]}
    )
    if cfg.dark_mode:
    ...
    ax1.fill_between(
        cycles, 0, v_inv, alpha=0.7, color=RED, label="V_inv (invariants)  w=0.40"
    )
    ax1.fill_between(
        cycles,
        v_inv,
    ...
    ax1.legend(
        fontsize=9, loc="upper right", facecolor=SURF, edgecolor=BORDER, labelcolor=TEXT
    )
...
def plot_basin_map(
    ...
    ax.contourf(
        X, Y, basin_map, levels=[-0.5, 0.5, 1.5, 2.5, 3.5], cmap=cmap, alpha=0.6
    )
    ...
    if trajectory:
        ...
        for i in range(len(tx) - 1):
            t = i / max(len(tx) - 1, 1)
            col = (1 - t) * np.array([0.34, 0.51, 0.93]) + t * np.array(
                [0.87, 0.45, 0.13]
            )
        ...
        ax.scatter(
            tx[0], ty[0], color=GREEN, s=120, zorder=6, label="Start", marker="o"
        )
        ax.scatter(
            tx[-1], ty[-1], color=ORANGE, s=150, zorder=6, label="End", marker="*"
        )
    ...
    ax.set_ylim(0, 0.8)
    ax.legend(
        fontsize=8, loc="upper right", facecolor=SURF, edgecolor=BORDER, labelcolor=TEXT
    )
...
def plot_separatrix_proximity(
    ...
    fig, (ax1, ax2) = plt.subplots(
        2, 1, figsize=(cfg.figsize[0], 6), gridspec_kw={"height_ratios": [2, 1]}
    )
    if cfg.dark_mode:
        _setup_dark(fig, [ax1, ax2])
        fig.suptitle(title, color=TEXT, fontsize=13, fontweight="bold")
    ...
    # Risk zone
    ax1.axhline(
        0.70, color=RED, lw=1.5, ls="--", alpha=0.8, label="At-risk threshold (0.70)"
    )
    ...
    # Ridge curvature κ
    ax2.bar(
        cycles, kappa, color=[RED if r else BLUE for r in at_risk], alpha=0.8, width=0.7
    )
